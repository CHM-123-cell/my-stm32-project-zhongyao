...
int main(void){
    ...
    Serial_Init();
    Servo_Init();Servo_Up(180);Servo_Lo(180);
    Sensor_Init();
    OLED_Init();OLED_Clear();OLED_ShowString(1,1,"Medicine V9.5");
    Servo_Up(135);Servo_Lo(135);Delay_ms(150);Servo_Up(180);Servo_Lo(180);
    OLED_Show();
    Serial_Printf("\r\n===== V9.5 =====\r\n");
    ...
    while(1){
        ...
        switch(st){
        case IDLE: if(run&&tg){run=0;Sensor_Up=0;Sensor_Lo=0;bad=0;Serial_Printf("[S] TGT=%d\r\n",tg);Servo_Up(90);Servo_Lo(180);to=now;tl=now;st=FEED;od=1;}break;
        case FEED: if(Sensor_Up>=tg){Servo_Up(180);Serial_Printf("[OK] UP=%d\r\n",Sensor_Up);t0=now;st=DROP;od=1;}
            else if((now-tl)>cm){Servo_Up(180);Serial_Printf("[TO] UP=%d/%d\r\n",Sensor_Up,tg);t0=now;st=DROP;od=1;}break;
        case DROP: if((now-t0)>=dm){Servo_Lo(90);to=now;st=VERIFY;Serial_Printf("[V]\r\n");od=1;}break;
        case VERIFY: if(Sensor_Lo>=Sensor_Up){Servo_Lo(180);Serial_Printf("[OK] LO=%d\r\n",Sensor_Lo);if(Sensor_Lo!=Sensor_Up)bad=1;blk=0;t0=now;st=BLINK;od=1;}
            else if((now-to)>cm){Servo_Lo(180);Serial_Printf("[TO] LO=%d/%d\r\n",Sensor_Lo,Sensor_Up);if(Sensor_Lo!=Sensor_Up)bad=1;blk=0;t0=now;st=BLINK;od=1;}break;
